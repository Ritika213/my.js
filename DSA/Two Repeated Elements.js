...
Note: //Return the numbers in their order of appearing twice. So, if X and Y are the repeating numbers, and X's second appearance comes 
//before second appearance of Y, then the order should be (X, Y).
...
//In the given array, 1 and 2 are repeated two times and second occurence of 2 comes before 1. So the output is 2 1.
 Task
//The task is to complete the function repeatedElements() which takes an integer array arr[] and an integer n as inputs (the size of 
    //the array is n + 2 and elements are in the range [1, n]) and finds the two repeated elements in the array and return them in an array.
...
class Solution
{
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (arr[index] < 0) {
      repeatingNumbers.push(Math.abs(arr[i]));
    } else {
      arr[index] = -arr[index];
    }
  }
