//Design a SpecialQueue data structure that functions like a normal queue but with additional support for retrieving the minimum and
//  maximum element efficiently.
...
class SpecialQueue {
    constructor() {
        this.queue = [];       // Normal queue
        this.minDeque = [];    // Monotonic increasing deque
        this.maxDeque = [];    // Monotonic decreasing deque
    }
    ...
    enqueue(x) {
        ...
        while (this.minDeque.length > 0 && this.minDeque[this.minDeque.length - 1] > x) {
            this.minDeque.pop();
        }
        this.minDeque.push(x);

        // Maintain maxDeque (monotonic decreasing)
        while (this.maxDeque.length > 0 && this.maxDeque[this.maxDeque.length - 1] < x) {
            this.maxDeque.pop();
        }
    ...
    getMin() {
        return this.minDeque.length > 0 ? this.minDeque[0] : null;
    }

    getMax() {
        return this.maxDeque.length > 0 ? this.maxDeque[0] : null;
    }
}
