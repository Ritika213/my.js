//Given two sorted arrays arr1 and arr2 and an element k. The task is to find the element that would be at the kth position of the combined 
//sorted array.
...
class Solution {
    kthElement(k, arr1, arr2) {
        ...
        for(let num of arr1)   // add elements of arr1 to ArrayList
            al.push(num);
            
        for(let num of arr2)   // add elements of arr1 to ArrayList
            al.push(num);
            
        al.sort((a,b)=>a-b);   // Sort ArrayList
        return al[k-1];
    }
}