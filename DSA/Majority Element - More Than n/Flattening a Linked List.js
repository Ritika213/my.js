...
//Each of the sub-linked lists nodes and the head nodes are sorted in ascending order based on their data. Flatten the linked 
// list such that all the nodes appear in a single level while maintaining the sorted order.
...
class Solution {
    mergeTwoSortedLL(l1, l2) {
        ...
        l1.bottom = this.mergeTwoSortedLL(l1.bottom, l2);
        return l1;
    }
    ...
    flatten(root) {
        ...
        let head2 = this.flatten(root.next);
        root.next = null; // important
        return this.mergeTwoSortedLL(root, head2);
    }
}
