...
Output: 1000
Explanation:// There is only one painter, so the painter must paint all boards sequentially. The total time taken will be the 
//sum of all board lengths, i.e., 100 + 200 + 300 + 400 = 1000.
Constraints
//1 ≤ arr.size() ≤ 105
//1 ≤ arr[i] ≤ 104
//1 ≤ k ≤ 105
...
class Solution {
    check(md, arr) {
        ...
        for (let i = 0; i < n; i++) {
            if (arr[i] > md) return Number.MAX_SAFE_INTEGER;
            sm += arr[i];
            if (sm > md) {
                k++;
                sm = arr[i];
    ...
    minTime(arr, k) {
        let l = 1, h = 10000000, ans = -1;

        while (l <= h) {
            let md = Math.floor(l + (h - l) / 2);

            if (k >= this.check(md, arr)) {
                ans = md;
                h = md - 1;
            } else {
                l = md + 1;
