//given an array of strings arr that represents a valid arithmetic expression written in Reverse Polish Notation (Postfix Notation). 
...
class Solution {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (operators.has(arr[i])) {
            let f = parseInt(stack.pop(), 10);
            let s = parseInt(stack.pop(), 10);
            ...
            if (arr[i] === "*") {
                ...
                stack.push(Math.trunc(s / f)); // Truncate for integer division
            }
        } else {
            stack.push(arr[i]);
        }
