//given an array arr[ ], where arr[i] represents the height of the ith person standing in a line.
...
class Solution {
    maxPeople(arr) {
        ...
        // store indices where arr[i] == mx
        for (let i = 0; i < n; i++) {
            if (arr[i] === mx) lt.push(i);
        }

        lt.push(n); // sentinel

        let pr = 0, cr, nxt, ans = 0;

        while (lt.length > 1) {
            cr = lt.shift();      // remove first
            nxt = lt[0];          // get first
