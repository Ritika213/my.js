//Given an array arr[] , return the final array by repeatedly apply the following operation from left to right until no more
//  valid operations can be performed.
...
//                Input: arr[] = [5, -5, -2, -10]
Output: [-2, -10]
Explanation:// 1st and 2nd element gets discarded because both elements have same values but opposite sign. 3rd and 4th elements
// have same sign. So, both will be in the array.
Input: arr = [-20, 1, 20]
Output: []
Explanation:// 1st and 2nd elements are removed, and -20 is inserted since it has the larger absolute value. Then, the remaining 
//elements [-20, 20] are removed (equal absolute values, opposite signs).
        ...
        for (let num of arr) {
            ...
            while (stack.length > 0) {
                ...
                if ((top > 0 && current < 0) || (top < 0 && current > 0)) {
                    ...
                    // Case 2: top has greater absolute value
                    if (Math.abs(top) > Math.abs(current)) {
                        current = top; // top survives
                        stack.pop();
                    } else {
                        // Case 3: current survives
                        stack.pop();
