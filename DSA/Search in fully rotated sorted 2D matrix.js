//given a 2D matrix mat[][] of size n x m that was initially filled in the following manner:
...
//Later, this sorted 1D array was rotated at some unknown pivot. The rotated array was then written back into the matrix row-wise
// to form the current matrix.
...
class Solution {
    searchMatrix(mat, x) {
        ...
        for (let i = 0; i < rows; i++) {
            // optional: sort row (only if needed — many matrix search problems assume rows are already sorted)
            mat[i].sort((a, b) => a - b);

            // quick bounds check
            if (x >= mat[i][0] && x <= mat[i][cols - 1]) {
                // linear search inside row
