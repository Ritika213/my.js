//Given a Binary Tree of n nodes, find all the nodes that don't have any siblings. You need to return a list of integers containing all 
//the nodes that don't have a sibling in sorted order (Increasing).
...
Task // don't need to read input or print anything. Complete the function noSibling() which takes the root of the tree as input parameter
 //and returns a list of integers containing all the nodes that don't have a sibling in sorted order.
 //If all nodes have a sibling, then the returning list should contain only one element -1.
