//Given an array arr[] of integers, for each element, find the closest (distance wise) to its right that has a higher frequency 
...
class Solution {
    findGreater(arr) {
        ...
        for (let i = 0; i < n; i++) {
            while (st.length > 0 && mp.get(arr[i]) > mp.get(arr[st[st.length - 1]])) {
