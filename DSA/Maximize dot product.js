//Given two arrays a and b of positive integers of size n and m where n >= m, the task is to maximize the dot product by inserting 
//zeros in the second array but you cannot disturb the order of elements.
...
//Maximum Dot Product = 1*0 + 2*0 + 3*4 = 12.
 Task  
//You don't need to read input or print anything. Complete the function maxDotProduct() which takes n, m, array a and b as input 
        ...
        for (let i = 1; i < n + 1; i++) {
            ...
            for (let j = 1; j < m + 1; j++) {
                if (i === 0 || j === 0) continue;
                    ...
                    curr[j] = Math.max(a[i - 1] * b[j - 1] + pre[j - 1], pre[j]);
            }
            pre = curr;
        }
        return pre[m]; 
