//Given a string str, the task is to find the bracket numbers, i.e., for each bracket in str, return i if the bracket is the ith opening or 
...
Input:  str = "(((()("
Output: //1 2 3 4 4 5
Explanation// The highlighted brackets in the given string (((()( are assigned
...
class Solution {
    bracketNumbers(str) {
        ...
        for (let i = 0; i < str.length; i++) {
            if (str[i] == '(') {
                // Assign number to opening bracket and push to stack
                v.push(k);
                st.push(k);
                k++;
            } else if (str[i] == ')') {
                // Assign number from stack to closing bracket and pop stack
                v.push(st.pop());
            }
        }
        return v;
