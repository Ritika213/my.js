//Given an integer array arr[] and a 2D array queries[][]. Each query queries[i] = [l, r] represents a subarray ranging from index 
// l to r (inclusive). For every query, compute the mean (average) of the elements in the specified range, and return the floor value 
// of that mean.
...
class Solution {
    findMean(arr, queries) {
        ...
        for (let i = 0; i < queries.length; i++) {
            ...
            let rangeSum;
            if (l === 0) rangeSum = pref[r];
            else rangeSum = pref[r] - pref[l - 1];

            let len = r - l + 1;
            ans.push(Math.floor(rangeSum / len)); 
        }
